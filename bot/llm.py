...
Пиши по-русски, тепло, спокойно, без эзотерического пафоса и без запугивания.
...
def build_asc_day_prompt(
    *,
    ...
    lines = [
        "Сделай персональный «День по асценденту» на сегодня.",
        f"Асцендент: {emoji} {sign}, {degree}° в знаке. Место рождения: {place}.",
    ...
    if card:
        ...
        lines.append("Сшей тон ASC с посланием карты.")
    lines.append(
        "Структура:\n"
...
def fallback_spread_text(cards: list[dict[str, Any]], title: str) -> str:
    ...
    return (
        ...
        + " ".join(bits[:2])
        + f"\n\n💫 {advice}\n\n"
        "_Офлайн-режим: ИИ временно недоступен (лимит free-модели). Повторите через минуту._"
    )
